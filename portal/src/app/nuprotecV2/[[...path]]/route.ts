...
async function proxy(req: NextRequest) {
  const client = getClient("nuprotecV2");
  if (!client || !client.proxyTarget) {
    return new Response("Cliente no configurado", { status: 404 });
  }

  // Sin puerta del portal: la réplica de Nuprotec tiene login propio, con
  // usuarios y roles en su base, así que pedir además la clave del portal
  // sería un doble login. Se reenvía directo y la app decide el acceso.
